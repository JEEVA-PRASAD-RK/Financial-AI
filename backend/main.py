...
#  API CONFIG
# ─────────────────────────────────────────────────────────
OPENROUTER_API_KEY = "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"
...
# Global conversation memory per user
...
def get_user_conversation(user_id):
    if user_id not in user_conversations:
        user_conversations[user_id] = [
            {"role": "system", "content": "You are a smart Indian financial assistant."}
        ]
    return user_conversations[user_id]


def build_personalized_prompt(user, profile):
    return f"""
User Details:
Name: {user.get('full_name')}
Income: ₹{profile.get('monthly_income', 0)}
Expense: ₹{profile.get('monthly_expense', 0)}
Field: {profile.get('work_field', 'Unknown')}
Insurance: {profile.get('has_insurance', 'Unknown')}
Emergency Fund: {profile.get('emergency_fund', 'Unknown')}

You are Finace AI.
Give short, practical, personalized financial advice for India.
"""        
...
def login(email: str = Form(...), password: str = Form(...)):
    ...
    if profile_rows:
       ...
       expense = float(r.get("monthly_expense") or 0)
       has_insurance = str(r.get("has_insurance") or "").lower()
       emergency_fund = str(r.get("emergency_fund") or "").lower()
...
@app.post("/chat")
async def chat(request: Request, message: str = Form(...), file: Optional[UploadFile] = File(None)):
    ...
    if not user_id:
        return {"reply": "Invalid or expired token"}

    # ✅ Use user-specific conversation
    ...
    if profile_rows:
        ...
        result = calculate_financials(income, expense)
        ...
        if result:
            ...
            - Emergency Fund: {result['emergency']}%
    ...
    # ✅ Add system message once per user
    if len(conversation) == 0:
       conversation.append({
           "role": "system",
           "content": f"""
    You are a smart financial assistant for Indian users.

    User Profile:
    - Income: ₹{income}
    - Expense: ₹{expense}

    Always give personalized advice based on this.
    """
       })
    ...
    user_conversations[user_id] = [conversation[0]] + conversation[-9:]
...
def calculate_financials(income, expense):
    if not income or income <= 0:
        return None

    remaining = income - expense
    rem_ratio = remaining / income if income > 0 else 0

    if remaining <= 0:
        return {"remaining": remaining, "savings": 0, "emergency": 0, "investment": 0}

    elif rem_ratio < 0.10:
        return {"remaining": remaining, "savings": 70, "emergency": 30, "investment": 0}

    elif rem_ratio < 0.30:
        return {"remaining": remaining, "savings": 40, "emergency": 30, "investment": 30}

    else:
        return {"remaining": remaining, "savings": 30, "emergency": 30, "investment": 40}
    
def calculate_health_score(income, expense, has_insurance, emergency_fund):
    if not income or income <= 0:
        return 0
...
def generate_suggestions(income, expense, has_insurance, emergency_fund):
    suggestions = []

    remaining = income - expense

    if remaining <= 0:
        suggestions.append("How can I reduce my expenses?")
    else:
        suggestions.append("How should I use my remaining money?")

    if expense > income * 0.7:
        suggestions.append("Am I spending too much?")

    if emergency_fund != "yes":
        suggestions.append("How to build an emergency fund?")

    if has_insurance != "yes":
        suggestions.append("Do I need insurance?")

    if remaining > income * 0.3:
        suggestions.append("Where should I invest my money?")
